...
void  calc_points_step(float *x_step, t_texture *t_step, t_point *p, t_texture *t, float delta)
{
	...
	t_step[1].u = (t[0].u - t[1].u) / delta;
	t_step[1].v = (t[0].v - t[1].v) / delta;
	t_step[1].w = (t[0].w - t[1].w) / delta;

	t_step[2].u = (t[0].u - t[2].u) / delta;
	t_step[2].v = (t[0].v - t[2].v) / delta;
	t_step[2].w = (t[0].w - t[2].w) / delta;
}

t_texture calc_step_texture(t_texture *t, float delta)
{	
	t_texture step;
	step.u = ((t[2].u / t[2].w) - (t[1].u / t[1].w)) / delta;
	step.v = ((t[2].v / t[2].w) - (t[1].v / t[1].w)) / delta;
