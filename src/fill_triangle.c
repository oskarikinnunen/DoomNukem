...
/*   Updated: 2022/12/01 17:18:42 by vlaine           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void fill_tri_bot(t_sdlcontext sdl, t_triangle triangle, t_img *img)
{
	...
	q = triangle.p;
	t = triangle.t;
	calc_step(x_step, t_step, triangle, 1.0f / (q[1].v.y - q[0].v.y)); // delta wrong using same delta for both q[2] and q[1]
	y = q[1].v.y;
	while (q[0].v.y <= y)
	{
...
static void fill_point_tri_top(t_sdlcontext sdl, t_point_triangle triangle, t_img *img)
{
	...
	delta = 1.0f/((float)p[0].y - (float)p[1].y);
	t_step[0].u = flerp(t[0].u, t[1].u, delta);
	t_step[0].v = flerp(t[0].v, t[1].v, delta);
	t_step[0].w = flerp(t[0].w, t[1].w, delta);
	step[0] = flerp(p[0].x, p[1].x, delta);

	delta = 1.0f/((float)p[0].y - (float)p[2].y);
	t_step[1].u = flerp(t[0].u, t[2].u, delta);
	t_step[1].v = flerp(t[0].v, t[2].v, delta);
	t_step[1].w = flerp(t[0].w, t[2].w, delta);
	step[1] = flerp(p[0].x, p[2].x, delta);

	y = p[1].y;
	while (y < p[0].y) // <=
	{
		delta = 1.0f / ((float)p[2].x - (float)p[1].x);
		t_step[2].u = (t[2].u - t[1].u) * delta;
...
void	z_fill_point_tri(t_sdlcontext sdl, t_point_triangle triangle, t_img img)
{
	t_point			p_split;
	t_texture		t_split;
	t_texture		t_temp;
