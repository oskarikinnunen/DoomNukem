...
/*   Updated: 2023/03/20 17:22:31 by raho             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	unpack_and_load_font(int font_i, char *level_path,
									char *font_name, t_sdlcontext *sdl)
{
	load_and_write_filecontent(level_path, font_name, TEMPFONT);
...
static int	parse_font_list(int fd, char *level_path, t_sdlcontext *sdl)
{
	int		ret;
	int		i;
	char	*font_name;
	...
	while (ret)
	{
		if (font_name)
		{
			unpack_and_load_font(i, level_path, font_name, sdl);
			free(font_name);
			font_name = NULL;
			i++;
		}
...
void	playmode_load_fonts(char *level_path, t_sdlcontext *sdl)
{
	...
	load_and_write_filecontent(level_path, FONTLISTPATH, TEMPFONTLIST);
	...
	ret = parse_font_list(fd, level_path, sdl);
	if (ret == -1)
