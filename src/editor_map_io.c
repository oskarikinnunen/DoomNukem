...
t_list *load_chunk(char *filename, char *chunkname, size_t size)
{
	...
	if (fd == -1)
	{
		printf("CHUNK LOAD %s from file %s failed!  fd -1\n", chunkname, filename);
		return (NULL);
	}
...
t_filecontent	load_filecontent(char	*worldname, char	*fc_name)
{
	...
	fd = open(worldname, O_RDONLY | O_BINARY);
	if (fd == -1)
		return (fc);
...
int load_filecontent_fd(char	*worldname, char *fcname)
{
	...
	fd = open(fcname, O_CREAT | O_RDWR | O_APPEND | O_BINARY, 0666);
...
void	save_filecontent(char	*worldname, char *filename)
{
	...
	close(fd);
	printf("file read '%s' \n", fc.content);
	fd = open(worldname, O_RDWR | O_APPEND | O_BINARY, 0666);
	if (fd == -1)
		return ;
	write(fd, "FCNK", 4);
