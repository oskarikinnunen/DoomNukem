...
/*   Updated: 2022/11/21 20:29:00 by raho             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	load_ttfonts(t_ttfont *ttfont)
{
	...
	ttfont->font_sizes[0] = TTF_OpenFont(ttf_file, 30); // last argument is font size
	if (!ttfont->font_sizes[0])
		error_log(EC_TTF_OPENFONT);
	ttfont->font_sizes[1] = TTF_OpenFont(ttf_file, 35);
	if (!ttfont->font_sizes[1])
		error_log(EC_TTF_OPENFONT);
	ttfont->font_sizes[2] = TTF_OpenFont(ttf_file, 40);
	if (!ttfont->font_sizes[2])
		error_log(EC_TTF_OPENFONT);
	ttfont->font_sizes[3] = TTF_OpenFont(ttf_file, 45);
	if (!ttfont->font_sizes[3])
		error_log(EC_TTF_OPENFONT);
	ttfont->font = ttfont->font_sizes[1]; // initialize the font pointer to the smallest font
	initialize_colors(ttfont);
	ttfont->color = ttfont->font_colors.crimson; // initialize the font's color to crimson
