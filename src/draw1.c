...
/*   Updated: 2022/12/01 12:26:29 by okinnune         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_text_temp(t_sdlcontext *sdl, const char *str, t_point pos, t_point boundaries)
{
	int			i;
	int			j;
	int			k;
	...
	while (str[i] != '\0')
	{
		...
		while (counter.y < sdl->font->chars[j].size.y)
		{
			...
			while (counter.x < sdl->font->chars[j].size.x)
			{
				...
				if (sdl->font->bitmap->data[(sdl->font->chars[j].pos.x + counter.x) + ((sdl->font->chars[j].pos.y + counter.y) * sdl->font->bitmap->size.x)] != 0 ||
					(sdl->font->bitmap->data[(sdl->font->chars[j].pos.x + counter.x) + ((sdl->font->chars[j].pos.y + counter.y) * sdl->font->bitmap->size.x)] != 0 && (screen.x % 2 == 0 && screen.y % 2 == 0)))
					draw(*sdl, point_add(screen, pos), INT_MAX);
				counter.x++;
			}
			counter.y++;
		}
...
t_rectangle	draw_text_black(t_sdlcontext *sdl, const char *str, t_point pos, t_point boundaries)
{
	...
	// Draw the background
	draw_rectangle_filled(*sdl, rect, CLR_DARKGRAY);
	// Draw the text
	draw_text_temp(sdl, str, pos, boundaries);
	return (rect);
}
